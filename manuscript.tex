...
\For{$k \leftarrow 0$ \KwTo $16$}{
    ...
    \lIf{$f^*[k] < 0$}{$f^*[k] \leftarrow 0$} \tcp*{Nicht-Negativit\"at sicherstellen}
}
\caption{Integer-Relaxation (17 Subtraktionen + 17 Multiplikationen + 17 Shifts)}
\end{algorithm}
...
\ForEach{Agent $A$}{
    ...
    $u_x \leftarrow \alpha \cdot c_{\kappa_Z,x}
    - \beta \cdot \sum_{j:\,d^2_j < d^2_{\min}} (d^2_{\min}/d^2_j) \cdot c_{\kappa_j,x}
    + \gamma \cdot \sum_{j:\,d^2_j > d^2_{\max}} (d^2_j/d^2_{\max}) \cdot c_{\kappa_j,x}$\;
    $u_y \leftarrow$ \text{analog f\"ur $y$-Komponente}\;
    \BlankLine
